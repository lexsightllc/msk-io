...
class FileEventHandler(FileSystemEventHandler):
    def __init__(self, processing_queue: queue.Queue, supported_extensions: list[str]):
        ...
        logger.info(
            f"File event handler initialized. Supported extensions: {self.supported_extensions}"
        )

    def _is_supported_file(self, event_path: str) -> bool:
        return any(
            event_path.lower().endswith(ext) for ext in self.supported_extensions
        )
    ...
    def on_modified(self, event: FileModifiedEvent) -> None:
        if (
            ...
            and self._is_supported_file(event.src_path)
            and not event.src_path.endswith((".tmp", "~"))
        ):
            logger.debug(
                f"Detected modified file (for potential reprocessing/completion): {event.src_path}"
            )
            pass
...
class DirectoryMonitor:
    def __init__(
        self,
        config,
        file_processor_callback: Callable[[str], Any],
        ...
        if not os.path.isdir(self.watch_directory):
            try:
                ...
                logger.warning(
                    f"Watch directory '{self.watch_directory}' did not exist and was created."
                )
            except OSError as e:
                raise ConfigurationError(
                    f"Watch directory '{self.watch_directory}' does not exist and cannot be created: {e}"
                ) from e
    ...
    def start_monitoring(self) -> None:
        ...
        event_handler = FileEventHandler(
            self.processing_queue, self.supported_extensions
        )
        ...
        self.worker_thread = threading.Thread(
            target=self._process_files_from_queue, daemon=True
        )
    ...
    def stop_monitoring(self) -> None:
        ...
        if self.worker_thread:
            ...
            if self.worker_thread.is_alive():
                logger.warning(
                    "File processing worker thread did not terminate cleanly."
                )
