...
from msk_io.errors import DataValidationError, ImageProcessingError
...
class Segmentor:
    ...
    def segment_image_volume(
        self, dicom_volume: DICOMVolume, threshold_value: float | None = None
    ) -> ImageSegmentationResult:
        logger.warning(
            f"Basic Segmentor.segment_image_volume is a conceptual stub. Simulating segmentation for {dicom_volume.volume_path}."
        )
        ...
        dummy_image_data = np.random.randint(
            0, 4000, size=(rows, cols), dtype=np.uint16
        )
