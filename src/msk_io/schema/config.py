...
class LLMSettingsSchema(MSKIOBaseModel):
    ...
    google_api_key: SecretStr | None = Field(None, description="Google Cloud API key.")
...
class ImageProcessingSettingsSchema(MSKIOBaseModel):
    ...
    segmentation_confidence_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Minimum confidence for segmentation masks.",
    )
...
class RetrievalSettingsSchema(MSKIOBaseModel):
    dicom_sniff_url: str = Field(
        "http://localhost:8042/dicom",
        description="Base URL for DICOM stream sniffing.",
    )
    ohif_extractor_headless: bool = Field(
        True, description="Run OHIF extractor in headless mode."
    )
    data_download_dir: str = Field(
        "/data/msk_io_downloads",
        description="Directory for downloaded medical data.",
    )
...
class AppSettingsSchema(MSKIOBaseModel):
    ...
    log_file_path: str | None = Field(
        None, description="Path to the application log file."
    )
    watch_directory: str = Field(
        "/data/msk_io_input",
        description="Directory to monitor for new medical data.",
    )
...
class AppConfigSchema(MSKIOBaseModel):
    app: AppSettingsSchema = Field(default_factory=lambda: AppSettingsSchema())
    llm: LLMSettingsSchema = Field(default_factory=lambda: LLMSettingsSchema())
    image_processing: ImageProcessingSettingsSchema = Field(
        default_factory=lambda: ImageProcessingSettingsSchema()
    )
    retrieval: RetrievalSettingsSchema = Field(
        default_factory=lambda: RetrievalSettingsSchema()
    )
    indexer: IndexerSettingsSchema = Field(
        default_factory=lambda: IndexerSettingsSchema()
    )
