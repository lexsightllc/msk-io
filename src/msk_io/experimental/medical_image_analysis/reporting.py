...
def _generate_text_report(
    ...
    with open(report_filename, "w") as f:
        ...
        f.write(
            f"   Processed Image Size: {processed_image_np.shape[1]}x{processed_image_np.shape[0]}\n"
        )
        f.write(f"   Imaging Modality: {MODALITY}\n")
        f.write(
            "   Processing Steps: Grayscale conversion, intensity normalization, resizing\n"
        )
        f.write(
            f"   Segmentation Quality: {_assess_segmentation_quality(segmented_mask_np)}\n\n"
        )

        f.write("2. SEGMENTATION RESULTS\n")
        f.write("-" * 30 + "\n")
        f.write(
            f"   Number of Detected Regions: {features.get('num_segmented_regions', 0)}\n"
        )
        f.write(
            "   Segmentation Method: Canny edge detection + morphological operations\n"
        )
        f.write(
            f"   Primary ROI Area: {features.get('total_area_pixels', 0):.0f} pixels\n"
        )
        f.write(
            f"   ROI Coverage: {_calculate_roi_coverage(segmented_mask_np):.1%} of image\n"
        )
        ...
        f.write(
            f"     - Perimeter: {features.get('total_perimeter_pixels', 0):.2f} pixels\n"
        )
        ...
        f.write(
            f"     - Intensity Std Dev: {features.get('intensity_std_dev', 0):.2f}\n"
        )
        ...
        f.write(
            f"     - Dissimilarity: {features.get('texture_dissimilarity', 0):.4f}\n"
        )
        ...
        f.write("-" * 30 + "\n")
        f.write(
            f"   Primary Classification: {classification_result.get('primary_classification', 'Unknown')}\n"
        )
        f.write(
            f"   Confidence Score: {classification_result.get('confidence_score', 0):.1%}\n"
        )
        f.write(
            f"   Risk Level: {classification_result.get('risk_level', 'Unknown')}\n\n"
        )
...
def _generate_annotated_image(
    ...
    annotated_image_filename = os.path.join(
        output_directory, f"annotated_image_{timestamp}.png"
    )
...
def _generate_feature_plots(
    features: dict, output_directory: str, timestamp: str
) -> None:
