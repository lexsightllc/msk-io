...
def harmonizer_instance(test_config):
    with (
        patch(
            "msk_io.control.multi_agent_harmonizer.RetrievalAgent", autospec=True
        ) as MockRetrievalAgent,
        patch(
            "msk_io.control.multi_agent_harmonizer.ImageProcessingAgent", autospec=True
        ) as MockImageProcessingAgent,
        patch(
            "msk_io.control.multi_agent_harmonizer.LLMInferenceAgent", autospec=True
        ) as MockLLMInferenceAgent,
        patch(
            "msk_io.control.multi_agent_harmonizer.SemanticIndexingAgent", autospec=True
        ) as MockSemanticIndexingAgent,
        patch(
            "msk_io.control.multi_agent_harmonizer.ReportingAgent", autospec=True
        ) as MockReportingAgent,
    ):
...
@pytest.mark.asyncio
async def test_run_task_pipeline_success(
    harmonizer_instance, mock_dicom_volume, test_config
):
    ...
    segmentation_result_data = ImageSegmentationResult(
        ...
        regions_of_interest=[
            RegionOfInterest(roi_id="roi1", label="lesion", pixel_count=100)
        ],
        segmentation_method="DL-Mock",
        processed_image_meta=ImageMetaData(
            original_path="dummy", image_format="nifti", dimensions=[1, 2, 3]
        ),
    ).model_dump()
    ...
    mocks["reporting"].execute_instruction.return_value = AgentResponse(
        ...
        output_data={
            ...
            "report_summary": {"overall_conclusion": "Mock final conclusion"},
        },
    )
    ...
    task_definition = TaskDefinition(
        ...
        sequence_of_instructions=[
            AgentInstruction(
                ...
                parameters={"patient_id": "test_patient"},
            ),
            AgentInstruction(
...
async def test_run_task_pipeline_failure_halts(harmonizer_instance, mock_dicom_volume):
    ...
    mocks["image_processing"].execute_instruction.return_value = AgentResponse(
        ...
        agent_name="image_processing",
        status="FAILED",
        error_message="Simulated image processing error.",
    )
...
async def test_agent_not_found(harmonizer_instance):
    ...
    task_definition = TaskDefinition(
        ...
        sequence_of_instructions=[
            AgentInstruction(
                command="DO_SOMETHING", target_agent="NonExistentAgent", parameters={}
            )
        ],
    )
    pipeline_status = await harmonizer.run_task_pipeline(task_definition)
    ...
    assert (
        "Target agent 'NonExistentAgent' not found" in pipeline_status.overall_message
    )
    assert len(pipeline_status.tasks_status) == 1
    assert pipeline_status.tasks_status[0].status == "FAILED"
    assert (
        pipeline_status.tasks_status[0].error_details["type"]
...
async def test_retrieval_agent_execute_instruction(test_config, mock_dicom_volume):
    ...
    with patch(
        "msk_io.retrieval.dicom_stream_sniffer.DICOMStreamSniffer.discover_and_retrieve_studies",
        MagicMock(),
    ) as mock_discover:
...
@pytest.mark.asyncio
async def test_image_processing_agent_execute_instruction(
    test_config, mock_dicom_volume
):
    agent = ImageProcessingAgent(test_config)
    with patch(
        "msk_io.image_processing.dl_segmentor.DLSegmentor.segment_image_volume",
        MagicMock(),
    ) as mock_dl_seg:
        mock_dl_seg.return_value = MagicMock(
            model_dump=lambda: {"mock": "dl_segmentation_result"}
        )
        instruction = AgentInstruction(
            command="PERFORM_DL_SEGMENTATION",
            parameters={"dicom_volume": mock_dicom_volume.model_dump()},
            target_agent="image_processing",
        )
        response = await agent.execute_instruction(instruction, {})
        assert response.status == "SUCCESS"
        assert response.output_data == {
            "segmentation_result": {"mock": "dl_segmentation_result"}
        }
...
async def test_llm_inference_agent_execute_instruction(test_config):
    ...
    mock_llm_agent.analyze_data.return_value = MagicMock(
        model_dump=lambda: {"mock": "llm_analysis_result"}
    )
    with (
        patch(
            "msk_io.inference.llm_agents.LLMAgentFactory.get_agent",
            return_value=mock_llm_agent,
        ),
        patch(
            "msk_io.inference.prompts.agent_prompts.get_prompt_template",
            return_value=MagicMock(format=lambda **kwargs: "formatted prompt"),
        ),
    ):
        instruction = AgentInstruction(
            command="ANALYZE_WITH_LLM",
            parameters={
                "agent_type": "mock_llm",
                "prompt_template_name": "MockTemplate",
        ...
        assert response.status == "SUCCESS"
        assert response.output_data == {
            "llm_analysis_result": {"mock": "llm_analysis_result"}
        }
...
async def test_semantic_indexing_agent_execute_instruction(test_config):
    ...
    with (
        patch(
            "msk_io.indexer.semantic_indexer.SemanticIndexer.index_document",
            MagicMock(),
        ) as mock_index_doc,
        patch(
            "msk_io.indexer.semantic_indexer.SemanticIndexer.query_semantic_index",
            MagicMock(),
        ) as mock_query_index,
    ):
        instruction_index = AgentInstruction(
            command="INDEX_DOCUMENT",
            parameters={"doc_id": "d1", "text_content": "some text"},
            target_agent="semantic_indexing",
        )
        ...
        instruction_query = AgentInstruction(
            command="QUERY_INDEX",
        ...
        assert response_query.output_data == {
            "query_results": [{"result": "doc_found"}]
        }
...
async def test_reporting_agent_execute_instruction(test_config, mock_dicom_volume):
        ...
        llm_output=MagicMock(
            choices=[
                MagicMock(
                    text=json.dumps(
                        {
                            "overall_conclusion": "LLM generated conclusion",
                            "recommendations": ["Do more tests"],
                        }
                    )
                )
            ]
    ...
    with (
        patch(
            "msk_io.inference.llm_agents.LLMAgentFactory.get_agent",
            return_value=mock_llm_agent_for_report,
        ),
        patch(
            "msk_io.inference.prompts.agent_prompts.get_prompt_template",
            return_value=MagicMock(format=lambda **kwargs: "formatted report prompt"),
        ),
    ):
        patient_info_dict = mock_dicom_volume.patient_info.model_dump()
        study_info_dict = mock_dicom_volume.study_info.model_dump()
        mock_img_analysis = ImageSegmentationResult(
            analyzed_volume=mock_dicom_volume,
            analysis_id="img_anal_1",
            qualitative_observations="img ok",
        ).model_dump()
        mock_llm_analysis = LLMAnalysisResult(
            llm_output=LLMOutput(
                response_id="llm_out_1",
                model_name_used="dummy",
                input_tokens=1,
                output_tokens=1,
                total_tokens=2,
                choices=[],
        ...
        instruction = AgentInstruction(
            ...
            parameters={
                ...
                "study_info": study_info_dict,
                "diagnostic_findings": [],
                "image_summaries_json": [mock_img_analysis],
                "llm_summaries_json": [mock_llm_analysis],
            },
            target_agent="reporting",
        )
        response = await agent.execute_instruction(instruction, {})
        ...
        assert (
            response.output_data["report_summary"]["overall_conclusion"]
        ...
        with open(response.output_data["report_path"]) as f:
            report_content = json.load(f)
            assert (
                report_content["patient_info"]["patient_id"]
